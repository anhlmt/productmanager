...
public class HoaDonBanHang extends javax.swing.JPanel {
    ...
    MyTable tb;
    int status = 1;
    Row tong, tienkhachtra, tienthua;
    
    public HoaDonBanHang() {
        initComponents();
        //
        tong = initRow();
        tienkhachtra = initRow();
        tienthua = initRow();
        tb = new MyTable(new String[]{"ID","Tên Sản Phẩm","Giá bán","Số lượng","Thành tiền","Check"},new int[]{Config.ID_WIDTH,Config.TEN_SAN_PHAM_WIDTH,Config.GIA_BAN_WIDTH,Config.SO_LUONG_WIDTH,Config.THANH_TIEN_WIDTH,Row.CHECK_BOX_WIDTH});
    ...
    public Row initRow(){
        Row row = new Row(new JComponent[]{new JTextField(),new JTextField(),new JTextField(),new JTextField(),new JTextField()}, new int[]{Config.ID_WIDTH,Config.TEN_SAN_PHAM_WIDTH,Config.GIA_BAN_WIDTH,Config.SO_LUONG_WIDTH,Config.THANH_TIEN_WIDTH});
        return row;
    }
    ...
    private void initComponents() {
        ...
        lb_status_ThanhToan.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseExited(java.awt.event.MouseEvent evt) {
                lb_status_ThanhToanMouseExited(evt);
            }
            public void mouseReleased(java.awt.event.MouseEvent evt) {
                lb_status_ThanhToanMouseReleased(evt);
            }
        });
        lb_status_ThanhToan.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyPressed(java.awt.event.KeyEvent evt) {
                lb_status_ThanhToanKeyPressed(evt);
            }
        });
    ...
    private void thanhtoan_btnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_thanhtoan_btnActionPerformed
        if(status == 1){
            ...
            // Thông báo
            if(Integer.parseInt(((JTextField)tienthua.components[4]).getText()) < 0){
    ...
    private void lb_status_ThanhToanKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_lb_status_ThanhToanKeyPressed
        // TODO add your handling code here:
    }//GEN-LAST:event_lb_status_ThanhToanKeyPressed

    private void lb_status_ThanhToanMouseExited(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_lb_status_ThanhToanMouseExited
        // TODO add your handling code here:
    }//GEN-LAST:event_lb_status_ThanhToanMouseExited

    private void lb_status_ThanhToanMouseReleased(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_lb_status_ThanhToanMouseReleased
        // TODO add your handling code here:
    }//GEN-LAST:event_lb_status_ThanhToanMouseReleased
    ...
    public void activeTinhTien(){
                ...
                tienkhachtra.chkbox.setVisible(false);
                ((JTextField)tienkhachtra.components[1]).setText("Tiền khách trả");
                ((JTextField)tienkhachtra.components[1]).setEditable(false);
                ((JTextField)tienkhachtra.components[4]).setEditable(true);
                ((JTextField)tienkhachtra.components[4]).addKeyListener(new KeyAdapter() {
                    int moneythua = 0;
                    int money = 0;
                    public void keyReleased(KeyEvent evt) {
                            if(!((JTextField)tienkhachtra.components[4]).getText().equals("")){
                                money = Integer.parseInt(((JTextField)tienkhachtra.components[4]).getText());
                                moneythua = - sum_final + money;
                                ((JTextField)tienthua.components[4]).setText(Integer.toString(moneythua));
                            }else{
                                ((JTextField)tienthua.components[4]).setText("0");
                            }
                    }
                });
                tb.addRow(tienkhachtra);
                
                tienthua.chkbox.setVisible(false);
                ((JTextField)tienthua.components[1]).setText("Tiền thối");
                ((JTextField)tienthua.components[1]).setEditable(false);
                ((JTextField)tienthua.components[4]).setText("0");
                ((JTextField)tienthua.components[4]).setEditable(false);
                tb.addRow(tienthua);
    }
